...
contract("Request factory", async function(accounts) {
	it("should create a request with provided properties", async function() {
		...
		expect(requestData.paymentData.donationBenefactor).to.equal(accounts[1])

		expect(requestData.paymentData.donationOwed).to.equal(0)
		...
		expect(requestData.schedule.reservedWindowSize).to.equal(
			reservedWindowSize
		)
		...
		expect(
			await requestFactory.isKnownRequest(txRequest.address)
		).to.be.true
	})
